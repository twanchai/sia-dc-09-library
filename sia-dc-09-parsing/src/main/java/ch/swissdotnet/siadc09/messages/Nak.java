/*
 * Copyright (c) 2025 Swissdotnet SA
 *
 * This file is part of the SIA-DC-09 Library project.
 *
 * This source code is dual-licensed:
 * 1. Non-commercial use is permitted under the Polyform Noncommercial License 1.0.0
 *    https://polyformproject.org/licenses/noncommercial/1.0.0/
 * 2. Commercial use requires a separate commercial license.
 *    To inquire about licensing, please contact: info@swissdotnet.ch
 *
 * Unless required by applicable law or agreed to in writing, this software
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND.
 */
package ch.swissdotnet.siadc09.messages;

import java.util.Optional;

/**
 * The {@code Nak} class represents the Negative acknowledge message in SIA DC-09.
 * <p/>
 * The {@code DataMessage} is <u>NEVER</u> ciphered and always contains a static part ("NAK"0000R0L0[]).
 *
 * @author Vincent Pasquier <vincent.pasquier at swissdotnet.ch>
 */
public class Nak extends Message {

    Nak(final Builder builder) {
        super(builder);
    }

    @Override
    public boolean isCiphered() {
        return false;
    }

    @Override
    public int getSequence() {
        return 0;
    }

    @Override
    public Optional<String> getReceiverNumber() {
        return Optional.of("0");
    }

    @Override
    public String getAccountPrefix() {
        return "0";
    }

    @Override
    public String getAccountNumber() {
        return "0";
    }

    @Override
    public MessageType type() {
        return MessageType.NAK;
    }

}
